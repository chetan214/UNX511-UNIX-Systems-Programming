...
static ssize_t hardware_device_read(struct file *filp, char __user *buf, size_t len, loff_t *off)
{
    ...
    bytes_to_read = BUF_LEN - *off;
    if (len < bytes_to_read) bytes_to_read = len;
    if (copy_to_user(buf, buffer + *off, bytes_to_read) != 0) {
        return -EFAULT;
    }
    *off += bytes_to_read;
...
static long hardware_device_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
    switch (cmd) {
        case HARDWARE_DEVICE_HALT:
            is_halt = true;
            printk(KERN_INFO "Hardware-Device: HALT received\n");
            break;
        case HARDWARE_DEVICE_RESUME:
            is_halt = false;
            printk(KERN_INFO "Hardware-Device: RESUME received\n");
            break;
        default:
            return -EINVAL;
    }
